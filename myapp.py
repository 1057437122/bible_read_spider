				...
				for chapter in item['list_content']:
					...
					for detail in cur_chapter:
						if not detail['content']:
							continue
						detail_id = get_detail_id(volume_id,list_id,detail,db)
						if detail_id == -1:
							content_insert_sql = 'insert into detail (volume_id,list_id,chapter,chapter_num,section_num,version,content) values("'+str(volume_id)+'","'+str(list_id)+'","'+chapter['chapter_name']+'","'+str(detail['chapter'])+'","'+str(detail['section'])+'","'+detail['version']+'","'+detail['content']+'")'
